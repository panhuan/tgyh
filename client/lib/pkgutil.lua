...
local pkgutil = {}
local _debug, _info, _warn, _error = require("qlog").loggers("pkgutil")
function pkgutil.pkgname(filename)
	return filename:gsub("[.][^.]+$", ""):gsub("/", ".")
end
local util = require("util")
function pkgutil.replace(pkgname, source, loader)
	if pkgname == nil then
		pkgname = pkgutil.pkgname(source)
	end
	if loader == nil then
		loader = dofile
	end
