...
local TESTING = false
local buildlib = {}
buildlib.COPY_WITH_PATH = "mkdir -p $(dirname \"$(TARGET)\") && cp \"$(INPUT)\" \"$(TARGET)\""
buildlib.COPY = "cp \"$(INPUT)\" \"$(TARGET)\""
buildlib.autocreate_output_path = true
...
local build_vars = {}
buildlib.vars = build_vars
function buildlib.exec(cmd, dont_fail, echo)
	...
	if not TESTING then
		...
		if res ~= 0 and not dont_fail then
			...
			buildlib.quit("%s --> failed with code %d", _cmd, res)
		end
		return res
	end
end
...
function buildlib.shell(cmd, ...)
	return (shell_nl(cmd, ...):gsub([[

$]], ""))
end
local _shell = buildlib.shell
function buildlib.splitpath(path)
	local i = #path
...
local function _maybe_run_rule(relfile, rule, dep, opts)
	...
	if force or dep_is_newer(dep, file) then
		...
		if buildlib.autocreate_output_path and not TESTING then
			local res, err = mkparentdirs(file)
			if not res then
				buildlib.error("creating directory for: %s: %s", file, err)
				return true
			end
		end
		do
...
function buildlib.run(dir, odir, rules, opts)
	opts = opts or {}
...
function buildlib.warning(msg, ...)
	io.stderr:write("warning: ", msg:format(...), "\n")
	io.stderr:flush()
end
function buildlib.error(msg, ...)
	errors = errors + 1
	io.stderr:write("error: ", msg:format(...), "\n")
	io.stderr:flush()
end
function buildlib.quit(msg, ...)
	buildlib.error(msg, ...)
	os.exit(1)
end
function buildlib.copy(src, dest)
