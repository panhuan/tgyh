...
local URLCache = _M
_M.__index = URLCache
function URLCache.new(cacheName)
...
function URLCache.cacheFile(self, urlstr)
...
function URLCache.exists(self, urlstr)
	local f = self:cacheFile(urlstr)
	return file.exists(f)
end
function URLCache.clear(self)
...
function URLCache.remove(self, urlstr)
	local f = self:cacheFile(urlstr)
...
local function _DoRequest(self, urlstr, localFile, callback, force, userData)
	if not force and file.exists(localFile) then
		printf("URLCache: returning cached result for %s", urlstr)
	...
	self.task = task
	printf("URLCache: beginning fetch of %s", task.urlstr)
	task:httpGet(task.urlstr)
end
function URLCache._PumpTasks(self)
...
function URLCache.fetch(self, urlstr, callback, force, userData)
	local f = self:cacheFile(urlstr)
...
function URLCache.getContents(self, urlstr)
	local f = self:cacheFile(urlstr)
