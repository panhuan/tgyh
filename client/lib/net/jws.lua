...
function jws._selectKey(keyStore, kid)
  local key
  if kid == nil then
...
jws.ALG_TO_CRYPTO_ALG = {HS1 = "sha1", none = "none"}
function jws.decode(text, keyStore)
...
function jws.encode(header, body, keyStore)
  ...
  local hmac = crypto.hmac.new(jws.ALG_TO_CRYPTO_ALG[header.alg], key)
  hmac:update(str)
