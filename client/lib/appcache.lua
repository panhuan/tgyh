...
local _debug, _info, _warn, _error = require("qlog").loggers("appcache")
...
appcache.STATUS_UNCACHED = "UNCACHED"
appcache.STATUS_IDLE = "IDLE"
appcache.STATUS_CHECKING = "CHECKING"
appcache.STATUS_DOWNLOADING = "DOWNLOADING"
appcache.STATUS_UPDATEREADY = "UPDATEREADY"
appcache.STATUS_SWAPPING = "SWAPPING"
appcache.STATUS_OBSOLETE = "OBSOLETE"
...
function appcache.new(manifestURL, keyStore, basePath)
	...
	if success and type(result) == "table" and result.manifestURL == manifestURL then
		self = result
		self._status = appcache.STATUS_IDLE
	else
		self = {}
		...
		self.lastUpdate = nil
		self._status = appcache.STATUS_UNCACHED
	end
	self._path = path
...
function appcache.createSignedManifest(basePath, alg, keyStore, kid)
	local table_insert = table.insert
	local header = {
		kid = kid,
		alg = alg,
...
function appcache.createSignedSplitTestManifest(basePath, weights, alg, keyStore, kid)
	local table_insert = table.insert
	local header = {
		kid = kid,
		alg = alg,
...
local function _appcache_GET_next_file(self)
	...
	if #Q < 3 then
		...
		if updateReady then
			self._status = appcache.STATUS_UPDATEREADY
	...
	if self._status ~= appcache.STATUS_DOWNLOADING then
		self._status = appcache.STATUS_DOWNLOADING
...
function appcache:status()
	return self._status
end
function appcache:purge(deleteAll)
	...
	self.groups = {}
	self.splitGroup = nil
	if self._status ~= appcache.STATUS_OBSOLETE then
		self._status = appcache.STATUS_UNCACHED
	end
end
function appcache:busy()
	local status = self._status
	return status == appcache.STATUS_CHECKING or status == appcache.STATUS_DOWNLOADING or status == appcache.STATUS_SWAPPING
end
function appcache:queueSize()
	return #self._queue / 3
end
function appcache:lastUpdateTag()
...
local function _appcache_GET_manifest(task)
	...
	if responseCode == 404 or responseCode == 410 then
		self._status = appcache.STATUS_OBSOLETE
...
function appcache:update(roll)
	if self:busy() then
		return
	end
	self._status = appcache.STATUS_CHECKING
...
function appcache:retain(group, value)
...
function appcache:swap()
	if self._status ~= appcache.STATUS_UPDATEREADY then
		return false
	end
	self._status = appcache.STATUS_SWAPPING
...
function appcache:ready(embeddedPath)
	if self._status ~= appcache.STATUS_UNCACHED then
		if self._status ~= appcache.STATUS_IDLE then
