local metatable = {}
function metatable.inject(t, meta)
	if type(meta) ~= "table" then
		error("invalid metatable: " .. tostring(meta))
	end
		...
		do
			...
			if type(oldindex) == "table" then
				function mt.__index(t, k)
					return meta[k] or oldindex[k]
				end
			else
				function mt.__index(t, k)
					return meta[k] or oldindex(t, k)
				end
			end
		end
	else
...
function metatable.copyinto(t, meta)
	if meta == nil then
		return
	end
	...
	if type(meta.__index) == "table" and meta.__index ~= meta then
		metatable.copyinto(t, meta.__index)
	end
	metatable.copyinto(t, getmetatable(meta))
	return t
end
...
metatable.table_print = table_print
return metatable
