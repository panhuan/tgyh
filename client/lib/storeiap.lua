...
local _debug, _info, _warn, _error = require("qlog").loggers("storeiap")

local storeiap = {}
storeiap.TXN_PURCHASING = "PURCHASING"
storeiap.TXN_PURCHASED = "PURCHASED"
storeiap.TXN_RESTORED = "RESTORED"
storeiap.TXN_FAILED = "FAILED"
storeiap.TXN_CANCELLED = "CANCELLED"
storeiap.TXN_REFUNDED = "REFUNDED"
...
local function _handlePaymentTransactionUpdate(txn)
	...
	if txn.transactionState ~= storeiap.TXN_PURCHASING and txn.transactionState ~= storeiap.TXN_RESTORED then
	...
	elseif txn.transactionState == storeiap.TXN_RESTORED then
	...
	elseif txn.transactionState == storeiap.TXN_CANCELLED then
		...
		callback(false, productId, txn)
	elseif txn.transactionState == storeiap.TXN_REFUNDED then
		_error("Product " .. productId .. ": Refunded")
		callback(false, productId, txn)
	elseif txn.transactionState == storeiap.TXN_PURCHASING then
	else
...
local function _androidPurchaseResponseLoop()
	while #complete_purchase_txns > 0 do
		...
		if code == MOAIApp.BILLING_RESULT_OK then
			_debug("Product " .. productId .. ": Purchased")
			txn.transactionState = storeiap.TXN_PURCHASED
			_handlePaymentTransactionUpdate(txn)
			active_purchase_txns[productId] = nil
		elseif code == MOAIApp.BILLING_RESULT_USER_CANCELED then
			_debug("Product " .. productId .. ": Cancelled")
			txn.transactionState = storeiap.TXN_CANCELLED
			_handlePaymentTransactionUpdate(txn)
			active_purchase_txns[productId] = nil
		else
			txn.error = _androidBillingResponseStr(code)
			txn.transactionState = storeiap.TXN_FAILED
...
function storeiap.info(productIds, callback, cacheDuration)
...
function storeiap.infoSupported()
	return MOAIApp.requestProductIdentifiers ~= nil
end

function storeiap.enabled()
...
function storeiap.purchased(prodId)
	if prodId == true then
...
function storeiap.buy(prodId, callback)
	...
	if MOAIApp.requestPaymentForProduct ~= nil then
		...
		txn = {
			transactionState = storeiap.TXN_PURCHASING,
			payment = {productIdentifier = prodId, quantity = 1},
			transactionDate = os.date("!%Y-%m-%d %H:%M:%S")
		}
...
function storeiap.purchasing()
	return purchase_callback ~= nil
end

function storeiap.restore(item_callback, complete_callback)
...
function storeiap.init()
	purchases = PersistentTable.new("store_purchase_cache", true)
	if MOAIApp.requestProductIdentifiers ~= nil then
		_TXN_STATE_MAP[MOAIApp.TRANSACTION_STATE_CANCELLED] = storeiap.TXN_CANCELLED
		_TXN_STATE_MAP[MOAIApp.TRANSACTION_STATE_FAILED] = storeiap.TXN_FAILED
		_TXN_STATE_MAP[MOAIApp.TRANSACTION_STATE_PURCHASED] = storeiap.TXN_PURCHASED
		_TXN_STATE_MAP[MOAIApp.TRANSACTION_STATE_PURCHASING] = storeiap.TXN_PURCHASING
		_TXN_STATE_MAP[MOAIApp.TRANSACTION_STATE_RESTORED] = storeiap.TXN_RESTORED
		...
		_TXN_STATE_MAP[MOAIApp.BILLING_STATE_PURCHASE_CANCELED] = storeiap.TXN_CANCELLED
		_TXN_STATE_MAP[MOAIApp.BILLING_STATE_ITEM_PURCHASED] = storeiap.TXN_PURCHASED
		_TXN_STATE_MAP[MOAIApp.BILLING_STATE_ITEM_REFUNDED] = storeiap.TXN_REFUNDED
