...
public class HttpCMCC {
   /** Default HTTP port */
   private static final int DEFAULT_PORT = 8241;
   ...
   public void RunHttp() {
       ...
       try {
           ...
           // 设定服务器端的消息处理器
           acceptor.setHandler(new HttpCMCCHandler());
           
           // 绑定端口,启动服务器
           acceptor.bind(new InetSocketAddress(port));

           System.out.println("Server now listening CMCC on port " + port);
       } catch (Exception ex) {
           ex.printStackTrace();
       }
   }
