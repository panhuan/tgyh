...
import XuanQu.qh360.util.Pay;
import XuanQu.qh360.util.PayAppByQiHoo;
import XuanQu.qh360.util.PayAppInterface;

public class HttpQH360Handler extends IoHandlerAdapter {
	...
	public void sessionOpened(IoSession session) {
		...
		System.out
				.println("one Clinet Connect : " + session.getRemoteAddress());
	}
	...
	public void messageReceived(IoSession session, Object message) {
		...
		logMsg.append(SdkBillingServer.PID_QH360);
		...
		try {
			...
			PayAppInterface payApp = new PayAppByQiHoo();
			Pay qihooPay = new Pay(payApp);
			String _result = qihooPay.processRequest(paramMap);
			if (!_result.equals("ok")) {

				// Sign无效
				logMsg.insert(0, "<errcode=0&errmsg=Sign无效");
				logMsg.append("&cperror=404>");
				logger.debug(logMsg.toString());
				return;
			}
			
			if (!gateway_flag.equals("success")) {
				
				// 失败
				logMsg.insert(0, "<errcode=0&errmsg='gateway_flag!=success'");
				logMsg.append("&cperror=404>");
				logger.debug(logMsg.toString());
				return;
			}
	...
	public void testOrder() {
		...
		String app_key 		= "80af9517e90923035992a95a5f824ee3";
		String product_id 	= "1";
		String amount 		= "1";
		String app_uid 		= "03bceb90-0a12-40a0-b8f7-41629810f21a";
		String app_ext1 	= "";
		String app_ext2 	= "";
		String user_id 		= "953230423";
		String order_id 	= "1406177437118871041";
		String gateway_flag = "success";
		String sign_type 	= "md5";
		String app_order_id = "03bceb90-0a12-40a0-b8f7-41629810f21a#1#1403008715";
		String sign_return 	= "17885ae2bd7da376ac95540e0cfb045";
		String sign 		= "2750559455c47a62417100ce841d2719";

		// 特殊字符被转了, 比如# --> %23, 反转一下 zhanghuaming
		app_order_id = java.net.URLDecoder.decode(app_order_id);
		int nAmount = Integer.parseInt(amount);
		
		// 暂定方案,把订单都发送到1号服务器,然后通过数据库共享到其他服务器...
		String serverid = "1";
...
//			}
			...
			// 成功
			logMsg.insert(0, "<errcode=1&errmsg='gateway_flag != success'");
			logMsg.append("cperror=200>");
			logger.debug(logMsg.toString());
