package XuanQu.downjoy;

import java.net.InetSocketAddress;
import org.apache.mina.filter.codec.ProtocolCodecFilter;
import org.apache.mina.filter.logging.LoggingFilter;
import org.apache.mina.transport.socket.nio.NioSocketAcceptor;

import XuanQu.httpserver.HttpServerProtocolCodecFactory;

public class HttpDownjoy {
   /** Default HTTP port */
   private static final int DEFAULT_PORT = 8090;

   /** Tile server revision number */
   public static final String VERSION_STRING = "$Revision: 600461 $ $Date: 2007-12-03 18:55:52 +0900 (?, 03 12? 2007) $";
   
   public static final String MID = "343";
   public static final String GID = "1";
   public static final String SID = "1";
   public static final String MERCHANTKEY = "G21CILrK";
   public static final String DESKEY = "RzIxQ0lMcks=";
   
   public void RunHttp() {
       int port = DEFAULT_PORT;
       try {
           // Create an acceptor
           NioSocketAcceptor acceptor = new NioSocketAcceptor();

           // Create a service configuration
           acceptor.getFilterChain().addLast(
                   "protocolFilter",
                   new ProtocolCodecFilter(
                           new HttpServerProtocolCodecFactory()));
           acceptor.getFilterChain().addLast("logger", new LoggingFilter());
           acceptor.setHandler(new HttpDownjoyHandler());
           acceptor.bind(new InetSocketAddress(port));

           System.out.println("Server now listening downjoy on port " + port);
       } catch (Exception ex) {
           ex.printStackTrace();
       }
   }
}