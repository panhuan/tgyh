...
function exportToLua ( padding, forceSquare ) {
	...
	if ( folderPath &&  folderPath.length ) {
        ...
        { 
			...
			fl.trace (folderPath + document.name)
			
			var atlas = new TextureAtlas ( padding, forceSquare );
			atlas.createTextureAtlas ( document );
			...
			atlasDoc = atlas.createTextureAtlasDoc (  document );
...
function getLayerLua ( atlas, layer, idx ) {
	...
	for ( var i = 0; i < layer.frames.length; ++i ) {
		...
		if ( frameLua ) {
			...
			if ( frameLua.get ( 'id' ) > -1) {
				Keys++
			}
			idKeys++
		}
	}
	lua.set ( 'idKeys', idKeys);
	lua.set ( 'xKeys', Keys );
	lua.set ( 'yKeys', Keys );
	lua.set ( 'rKeys', Keys );
	lua.set ( 'sxKeys', Keys );
...
exportToLua ( MOAI_TEXTURE_ATLAS_PADDING, false );
