...
local FORMAT_HEADER_LINE       = "| %-50s: %-40s: %-10s: %-10s: %-10s: %-10s: %-10s|\n"
local FORMAT_OUTPUT_LINE       = "| %s: %-10s: %-10s: %-10s: %-10s|\n"
...
local FORMAT_TITLE             = "%-50.50s: %-40.40s: %-10s"
...
function ProFi:writeReportsToFile( file )
...
function ProFi:writeProfilingReportsToFile( reports, file )
	...
	local totalTime = stopTime - self.startTime
	local totalTimeOutput =  string.format(FORMAT_TOTALTIME_LINE, totalTime)
	file:write( totalTimeOutput )
	local header = string.format( FORMAT_HEADER_LINE, "FILE", "FUNCTION", "LINE", "TIME", "AVERAGE", "RELATIVE", "CALLED" )
	file:write( header )
 	for i, funcReport in ipairs( reports ) do
		local timer         = string.format(FORMAT_TIME, funcReport.timer)
		local average       = string.format(FORMAT_TIME, funcReport.timer / funcReport.count)
		...
		local outputLine    = string.format(FORMAT_OUTPUT_LINE, funcReport.title, timer, average, relTime, count )
