...
function csv.fromCSV(line, sep, detectnums)
...
function csv.rowiter(text, sep, detectnums)
	local sub = string_sub
	local find = string_find
	local table_insert = table.insert
	local len = text:len()
...
function csv.file_rowiter(filename, sep, detectnums)
	...
	f:close()
	return csv.rowiter(content, sep, detectnums)
end
function csv.file_totable(filenameOrIter, sep, detectnums)
	local rowIter
	if type(filenameOrIter) == "function" then
		rowIter = filenameOrIter
	else
		rowIter = csv.file_rowiter(filenameOrIter, sep, detectnums)
	end
	local rows = {}
	local ncols
	local table_insert = table.insert
...
function csv.file_torecordset(filename, sep, detectnums)
	local data = csv.file_totable(filename, sep, detectnums)
...
function csv.file_listrows(filenameOrIter, reqFields, sparseFields, sep, detectnums)
	local rowIter
	if type(filenameOrIter) == "function" then
		rowIter = filenameOrIter
	else
