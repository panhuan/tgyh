...
local proxy = {
	FLAG_PROXY_ACCEPT	= "#ACC",
	FLAG_PROXY_CLOSE	= "#CLS",
	FLAG_PROXY_DOWNWARD	= "#DNW",
	FLAG_PROXY_DISPATCH	= "#DIS",
}
setmetatable(proxy, {__index = network})
...
function proxy.launchGateway(ip, port, cb)
	network.connect(ip, port, function(c, e)
		if c then
