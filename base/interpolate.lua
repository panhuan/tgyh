...
function interpolate.lagrange2(x, y)
	local n = #x
...
function interpolate.lagrange2d(x, y, c)
	local n = #x
	local t = {}
	for i = 1, n do
		t[i] = c:getLength() * (i - 1) / (n - 1)
	end
	local fx = interpolate.lagrange2(t, x)
	local fy = interpolate.lagrange2(t, y)
	return function(t)
		local v = c:getValueAtTime(t)
		local resultX, resultY = fx, fy
		if type(fx) == "function" then
			resultX = fx(v)
		end
		if type(fy) == "function" then
			resultY = fy(v)
		end
...
function interpolate.makeCurveByTab(tb)
	local curve = MOAIAnimCurve.new()
	curve:reserveKeys(#tb)
	for key, var in ipairs(tb) do
		curve:setKey(key, var[1], var[2], var[3])
	end
	return curve
end
